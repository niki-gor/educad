...
TEST(AngemUtils, getPerpendicularFromPointToLine)
{
    PTR<Point> A(new PointByCoords(8, 6, 0));
    PTR<Point> B(new PointByCoords(6, 3, 0));
    PointByCoords C = PointByCoords(2, 11, 1);
    LineByTwoPoints l1 = LineByTwoPoints(A,B);
    AngemLine lr = AngemUtils::getPerpendicularLine(C,l1);
    EXPECT_DOUBLE_EQ(lr.i, -0.83);
    EXPECT_DOUBLE_EQ(lr.j, 0.55);
    EXPECT_DOUBLE_EQ(lr.k, 0.13);
    EXPECT_DOUBLE_EQ(lr.x0, 2);
    EXPECT_DOUBLE_EQ(lr.y0, 11);
...
TEST(AngemUtils, parallelLine)
{
    ...
    EXPECT_DOUBLE_EQ(lr.i, -0.1);
    EXPECT_DOUBLE_EQ(lr.j, 0.2);
    EXPECT_DOUBLE_EQ(lr.k, 0.98);
}
